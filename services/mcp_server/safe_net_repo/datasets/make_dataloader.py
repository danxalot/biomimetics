...
def make_dataset(opt):
    ...
    image_datasets = Dataloader_University(opt.data_dir, transforms=data_transforms, names=['satellite','drone'])
    samper = Sampler_University(image_datasets, batchsize=opt.batchsize, sample_num=opt.sample_num)
    dataloaders = torch.utils.data.DataLoader(image_datasets, batch_size=opt.batchsize, sampler=samper, num_workers=opt.num_worker, 
