...
DATA_HUB_URL = os.getenv('DATA_HUB_MCP_URL') or os.getenv('MCP_SERVER_URL') or 'http://mcp_server:8086'
print(f"DEBUG: Initialized with DATA_HUB_URL: {DATA_HUB_URL}")
logger.info(f"Initialized with DATA_HUB_URL: {DATA_HUB_URL}")
...
class MCPClient:
    ...
    def __init__(self, server_url: str, instance_id: str):
        ...
        self.api_key = os.getenv("MCP_API_KEY") or os.getenv("X_LITELLM_API_KEY") or os.getenv("LITELLM_API_KEY")
        self.default_headers = {}
        if self.api_key:
            # Header used in many GHCR/Proxy examples
            self.default_headers["x-litellm-api-key"] = f"Bearer {self.api_key}" if not self.api_key.startswith("Bearer ") else self.api_key
        self.session = None
    ...
    async def record_learning_event(self, skill_name: str, success: bool, context: str, details: Dict[str, Any] = None):
        """Record a learning event"""
        return await self.call_tool("record_learning_event", {
    ...
    async def analyze_skill_performance(self, skill_name: str, time_period: str = "30d") -> Dict[str, Any]:
        """Analyze skill performance"""
        return await self.call_tool("analyze_skill_performance", {
            "skill_name": skill_name,
    ...
    async def query_gordon_ai(self, prompt: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        return await self.call_tool("query_gordon_ai", {
            "prompt": prompt,
            "context": context or {}
        })
...
class SkillsAwareAgent:
    ...
    async def execute_with_skills_tracking(self, task_description: str, execution_func, *args, **kwargs):
        ...
        async with MCPClient(self.data_hub_url, self.instance_id) as mcp:
            ...
            logger.info(f"Recommended skills for '{task_description}': {recommended_skills}")
            ...
            try:
                # Execute the task
                result = await execution_func(*args, **kwargs) if asyncio.iscoroutinefunction(execution_func) else execution_func(*args, **kwargs)
                success = True
                ...
                if recommended_skills:
                    # Ensure all items are strings to prevent join errors
                    skill_list = [str(s.get('name', s) if isinstance(s, dict) else s) for s in recommended_skills[:10]]
                    skill_tag = ",".join(skill_list)
                else:
                    skill_tag = "no_skills_recommended"

                try:
                    await mcp.record_learning_event(skill_tag, success, context, details)
                    logger.info(f"Recorded batched learning event for skills: {skill_tag} -> {'success' if success else 'failure'}")
                except Exception as e:
    ...
    async def get_improvement_suggestions(self) -> Dict[str, Any]:
        ...
        async with MCPClient(self.data_hub_url, self.instance_id) as mcp:
            ...
            for skill in skills_needing_improvement[:3]:
                ...
                suggestions["learning_opportunities"].append(f"Practice {skill['name']} with simpler tasks first")
            
            return suggestions
    
    async def enhanced_reasoning(self, prompt: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
...
async def knowledge_processor_enhanced_analysis(data_description: str, analysis_func, data):
    """Knowledge processor with skills tracking"""
    agent = SkillsAwareAgent("knowledge-processor", DATA_HUB_URL)
    return await agent.execute_with_skills_tracking(data_description, analysis_func, data)

async def gateway_enhanced_coordination(coordination_task: str, coordination_func, *args):
    """Gateway instance with skills tracking"""
    agent = SkillsAwareAgent("gateway", DATA_HUB_URL)
    return await agent.execute_with_skills_tracking(coordination_task, coordination_func, *args)
            ...
            response["result"] = {
                "tools": [{
                    ...
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "task": {"type": "string", "description": "Natural language description of the task (e.g. 'Build Pythia image on OCI')"},
                            "agent_hint": {"type": "string", "description": "Optional hint for which agent to use (docker, git, serena)", "enum": ["docker", "git", "serena", "auto"]},
                            "instruct": {"type": "string", "description": "Optional per-job instructions for instruct models"}
                        },
                        "required": ["task"]
                    }
                }]
            ...
            if tool_name == "delegate_task":
                ...
                async def actual_execution():
                    async with MCPClient(DATA_HUB_URL, INSTANCE_ID) as mcp:
                        ...
                        if agent_hint in ["docker", "git", "serena", "code_maintainer", "security", "observer"]:
                        ...
                        logger.info(f"Delegating '{task}' -> dispatch_agent({agent_type})")
                        return await mcp.call_tool("dispatch_agent", {
                            "agent_type": agent_type,
                            "operation": operation,
                            "params": params,
                            "instruct": tool_args.get("instruct")
                        })

                result = await agent.execute_with_skills_tracking(task, actual_execution)
             ...
             # Fallback: Forward other MCP calls to the main server? 
             # No, strictly strictly enforce delegation via specific tools for safety
             raise ValueError(f"Method {method} not supported on Client Interface. Use 'delegate_task'.")
