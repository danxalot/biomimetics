    ...
    while True:
        try:
            if kernel and kernel.current_state:
                ...
                # Publish the full KernelState to Redis every tick as requested
                if redis_client:
                    try:
                        full_state = serialize_kernel_state(kernel.current_state)
                        redis_client.set("arca:geometry:current_state", json.dumps(full_state))
                    except Exception as redis_err:
                        logger.debug(f"Tick state publish: {redis_err}")
...
@app.route("/geometry/state", methods=["GET"])
...
@app.route("/geometry/simulate", methods=["POST"])
def simulate():
    """
...
@app.route("/geometry/validate", methods=["POST"])
def validate():
    """
...
@app.route("/geometry/apply", methods=["POST"])
def apply():
    """
    POST /geometry/apply
...
# ============================================================================
# INTAKE FOLDER WATCHER
# Auto-processes files dropped into shared_storage/ingest/intake/
# ============================================================================
_INTAKE_DIR = "/app/shared_storage/ingest/intake"
_PROCESSED_DIR = "/app/shared_storage/ingest/processed"
_DEFAULT_OBJECTIVE = os.environ.get("INTAKE_OBJECTIVE", "document analysis")
_INTAKE_POLL_INTERVAL = int(os.environ.get("INTAKE_POLL_INTERVAL", "15"))  # seconds


def _intake_watcher():
    """Background thread that polls the intake folder and auto-ingests new files."""
    os.makedirs(_INTAKE_DIR, exist_ok=True)
    os.makedirs(_PROCESSED_DIR, exist_ok=True)
    seen: set = set()
    logger.info(f"👁️  Intake watcher started — polling {_INTAKE_DIR} every {_INTAKE_POLL_INTERVAL}s")

    while True:
        try:
            entries = [
                f for f in os.listdir(_INTAKE_DIR)
                if os.path.isfile(os.path.join(_INTAKE_DIR, f))
                and not f.startswith(".")  # skip hidden/temp files
            ]
            for fname in entries:
                if fname in seen:
                    continue
                seen.add(fname)
                src = os.path.join(_INTAKE_DIR, fname)
                logger.info(f"📥 Intake: auto-ingesting {fname}")
                try:
                    import requests as _req
                    _req.post(
                        "http://localhost:8087/geometry/ingest_recursive",
                        json={
                            "file_path": f"ingest/intake/{fname}",
                    ...
                    # Move to processed
                    import shutil
                    shutil.move(src, os.path.join(_PROCESSED_DIR, fname))
                    logger.info(f"✅ Intake processed: {fname} → processed/")
                except Exception as ie:
                    logger.error(f"❌ Intake auto-ingest failed for {fname}: {ie}")
                    seen.discard(fname)  # retry next poll
        except Exception as we:
            logger.error(f"Intake watcher error: {we}")
        time.sleep(_INTAKE_POLL_INTERVAL)


_watcher_thread = threading.Thread(target=_intake_watcher, daemon=True, name="intake-watcher")
...
@app.route("/geometry/ingest_recursive", methods=["POST"])
def ingest_recursive():
    ...
    try:
        ...
        # PATH NORMALIZATION: Resolve user-facing paths to container paths
        # User says: /jobs/... or jobs/... → Container needs: /app/shared_storage/jobs/...
        original_path = file_path
        if file_path.startswith("/jobs/") or file_path.startswith("jobs/"):
            file_path = "/app/shared_storage/" + file_path.lstrip("/")
            logger.info(f"🔧 Path normalized: {original_path} → {file_path}")
        elif not file_path.startswith("/app/"):
            # Assume relative to shared_storage
            file_path = "/app/shared_storage/" + file_path.lstrip("/")
            logger.info(f"🔧 Path normalized: {original_path} → {file_path}")
        ...
        # ── Write output files to shared_storage/ingest/output/ ────────────
        try:
            safe_name = os.path.splitext(os.path.basename(file_path))[0]
            ts = datetime.utcnow().strftime("%Y%m%dT%H%M%S")
            out_dir = "/app/shared_storage/ingest/output"
            os.makedirs(out_dir, exist_ok=True)

            # 1. Full solar-system JSON
            ss_path = os.path.join(out_dir, f"{safe_name}_{ts}_solar_system.json")
            with open(ss_path, "w", encoding="utf-8") as fh:
                json.dump(solar_system, fh, indent=2, default=str)

            # 2. Raw vectors (position arrays from every object + trajectory)
            vectors_payload = {
                ...
                "object_vectors": [
                    {"id": obj.get("id"), "position": obj.get("position", [0, 0, 0])}
                    for obj in solar_system.get("objects", [])
                ],
            }
            vec_path = os.path.join(out_dir, f"{safe_name}_{ts}_Vectors.json")
            with open(vec_path, "w", encoding="utf-8") as fh:
                json.dump(vectors_payload, fh, indent=2, default=str)

            logger.info(f"📄 Output written: {ss_path} + {vec_path}")
        except Exception as ow:
            logger.warning(f"Failed to write output files: {ow}")
        ...
        if redis_client:
            try:
                ...
                # Also push to history for context tracking
                redis_client.lpush("arca:blackboard:geometry_history", json.dumps({
                    "system_id": solar_system.get("system_id"),
                    "timestamp": datetime.utcnow().isoformat(),
                    "object_count": len(solar_system.get("objects", []))
                }))
                redis_client.ltrim("arca:blackboard:geometry_history", 0, 19)  # Keep last 20
                
                # SET CONVERSATION FOCUS - Critical for context tracking
                focus_data = {
                    "subject": original_path,
                    "type": "document",
                    "objective": objective,
                    "object_count": len(solar_system.get("objects", [])),
                    "ttl": 10  # 10 conversation turns
                }
                redis_client.set("arca:conversation:focus", json.dumps(focus_data))
                logger.info(f"📍 Set conversation focus to: {original_path}")
...
@app.route("/geometry/analyze", methods=["POST"])
def analyze_query():
    """
    POST /geometry/analyze
    Agent tool endpoint: Analyze geometric state relative to a query.
    """
    try:
        data = request.get_json()
        query = data.get("query", "")
        # Placeholder: Compute similarity between query vector and graph nodes
        return jsonify({
            "analysis": "Geometric state indicates high stability around 'system_coherence'.",
            "relevant_nodes": list(kernel.current_state.nodes.keys()) if kernel and kernel.current_state else []
        }), 200
    except Exception as e:
...
@app.route("/geometry/force", methods=["POST"])
...
@app.route("/geometry/render", methods=["GET"])
def render():
    """
    GET /geometry/render?state_id=<uuid>&view=<view>&time_window=<N>

    Get visualization data.

    Query params:
      state_id: state to render (defaults to current)
      view: "concepts" | "attractors" | "energy" | "trajectories"
      time_window: number of timesteps to show (for trajectories)

    Returns mesh + metadata (no prose, human-readable visuals).
    """
    try:
        state_id = request.args.get("state_id")
        view = request.args.get("view", "concepts")
...
@app.route("/geometry/metrics", methods=["GET"])
def metrics():
    """
