...
class AgentWorkflowEngine:
    ...
    AGENT_CONFIG_MAP = {
        ...
        "feasibility_auditor": "feasibility_auditor",
        # Fast Genesis variants
        "fast_architect": "fast_architect",
        "fast_planner": "fast_planner",
        "fast_engineer": "fast_engineer",
    }
    ...
    def __init__(
        ...
        # 1. Load and validate configuration from SSOT (model_config.py)
        self.llm_configs = {}
        primary_llm_config = None

        if MODEL_CONFIG_AVAILABLE:
            logger.info("📡 Loading model configurations from shared/model_config.py (SSOT)")
            for key, cfg in MODEL_REGISTRY.items():
                ...
                self.llm_configs[key] = config_entry
                if cfg.is_primary:
                    primary_llm_config = config_entry
        else:
            logger.warning("⚠️ shared/model_config.py not available, falling back to JSON only")

        # 2. Load service-specific overrides from llm_config.json if it exists
        try:
            if os.path.exists(llm_config_path):
                logger.info(f"📂 Loading overrides from {llm_config_path}")
                with open(llm_config_path, "r") as f:
                    config = json.load(f)
                
                for llm in config.get("llms", []):
                    # Validate required fields for overrides
                    name = llm.get("name")
                    if name:
                        # Update or add to llm_configs
                        self.llm_configs[name] = {**self.llm_configs.get(name, {}), **llm}
                        if llm.get("is_primary"):
                            primary_llm_config = self.llm_configs[name]
        except Exception as e:
            logger.warning(f"Failed to load {llm_config_path}: {e}")

        # Ensure we have a primary LLM
        if not primary_llm_config and self.llm_configs:
            # Fallback to first available if no primary set
            primary_llm_config = list(self.llm_configs.values())[0]
            logger.warning(f"No primary LLM set, falling back to {primary_llm_config['name']}")
        
        if not primary_llm_config:
            raise ValueError("CRITICAL: No LLM configurations available from SSOT or JSON")
