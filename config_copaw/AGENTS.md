...
You MUST use the following MCP servers for specific tasks instead of generic local tools:

1. **Email Operations**: Use the Email MCP server tools (read_recent_emails, send_email) instead of himalaya or local file operations
2. **Code Analysis**: Use Serena MCP servers (serena_arca_mcp, serena_bios_mcp) for semantic code analysis instead of grep/find
3. **Knowledge Base**: Use Notion MCP server (notion_mcp) for BiOS Root sync operations
4. **Memory Operations**: Route all contextual queries (Health, Disability_Impact_Analysis, etc.) to the GCP Memory Gateway instead of local text files

DO NOT use generic local files for memory. You MUST route context queries to the GCP Memory Gateway.
DO NOT use himalaya for emails. You MUST use the local Email MCP Server tools.
