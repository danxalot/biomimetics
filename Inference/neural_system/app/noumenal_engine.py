    ...
    def absorb_pulse(self, vector_256: np.ndarray, coupling_strength: float = 0.15):
        ...
        for i, block in enumerate(self.blocks):
            if hasattr(block, 'hidden_state') and block.hidden_state is not None:
                ...
                if current_l2 < 1.0:
                    decay = 0.98  # Hypometabolic: Retain energy to build baseline
                elif current_l2 > 4.0:
                    decay = 0.80  # Hypermetabolic: Aggressive cooling to prevent "fever"
                else:
                    decay = 0.90  # Homeostatic Baseline
                
                # 3. Apply the calculated decay
                block.hidden_state *= decay
                
                # 4. Inject the Hamiltonian pulse
                block.hidden_state += (vector_256 * coupling_strength)
                
                # 5. Thermal Clamping: Hard safety limit to prevent divergence
                new_norm = np.linalg.norm(block.hidden_state)
                if new_norm > 5.0:
                    block.hidden_state = (block.hidden_state / new_norm) * 5.0
