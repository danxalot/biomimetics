...
# NOTE (2026-06-02): Per user direction, this canonical Azure credentials
# server does NOT expose Oracle wallet or DB endpoints. The memory_system
# handles its Oracle wallet ephemerally via
# services/oracle_utils/connection.py + the wallet files volume-mounted at
# ORACLE_WALLET_DIR. Nothing else in the cluster talks to the Oracle DBs.
# Wallet files should be uploaded to Azure Key Vault as secrets (see
# scratch/upload_oracle_wallet_to_azure.py) so they participate in the same
# single-source-of-truth policy as the rest of the secrets, but they are
# consumed by the memory_system via direct wallet reads, NOT via this server.
