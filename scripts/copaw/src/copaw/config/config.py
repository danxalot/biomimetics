...
class WhatsAppConfig(BaseChannelConfig):
    """WhatsApp channel (Green API) configuration."""

    instance_id: str = ""
    api_token: str = ""
    media_dir: str = "~/.copaw/media/whatsapp"
...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    relay_url: str = "ws://100.X.X.X:8765/ws/live"
    model: str = "models/gemini-3.1-flash-live-preview"


class ArcaBridgeConfig(BaseChannelConfig):
    """ARCA bridge channel: routes the CoPaw agent into the ARCA user-agent surface."""
...
class ChannelConfig(BaseModel):
    ...
    arca_bridge: ArcaBridgeConfig = ArcaBridgeConfig()
...
class ToolsConfig(BaseModel):
    ...
    builtin_tools: Dict[str, BuiltinToolConfig] = Field(
        default_factory=lambda: {
            ...
            "render_canvas": BuiltinToolConfig(
                name="render_canvas",
                enabled=True,
