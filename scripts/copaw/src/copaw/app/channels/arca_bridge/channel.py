...
class ArcaBridgeChannel(BaseChannel):
    """ArcaBridge Channel: routes agent responses to ARCA Bridge.
    ...
    def from_env(
        ...
        return cls(
            process=process,
            enabled=os.getenv("ARCA_BRIDGE_CHANNEL_ENABLED", "1") == "1",
            bot_prefix=os.getenv("ARCA_BRIDGE_BOT_PREFIX", "[BOT] "),
            on_reply_sent=on_reply_sent,
        )
    ...
    def from_config(
        ...
        """Create ArcaBridgeChannel from config.
        ...
        Returns:
            Configured ArcaBridgeChannel instance.
        """
        return cls(
            process=process,
    ...
    async def consume_one(self, payload: Any) -> None:
        ...
        try:
            ...
            async for event in self._process(request):
                ...
                logger.debug(
                    "arca_bridge event #%s: object=%s status=%s type=%s",
                    event_count,
                    obj,
                    status,
                    ev_type,
            ...
            logger.info(
                "arca_bridge stream done: event_count=%s has_response=%s",
                event_count,
            ...
            logger.exception("arca_bridge process/reply failed")
    ...
    async def start(self) -> None:
        if not self.enabled:
            logger.debug("arca_bridge channel disabled")
            return
        logger.info("ArcaBridge channel started")

    async def stop(self) -> None:
        if not self.enabled:
            return
        logger.info("arca_bridge channel stopped")
