...
class ChannelManager:
    ...
    def from_config(
        ...
        channels: list[BaseChannel] = []
        logger.info(f"Available channels: {available}")
        for key, ch_cls in get_channel_registry().items():
            logger.info(f"Checking channel key: {key}")
            if key not in available:
                logger.debug(f"Channel {key} not in available list")
                continue
            ch_cfg = getattr(ch, key, None)
            logger.info(f"Config for {key}: {ch_cfg}")
