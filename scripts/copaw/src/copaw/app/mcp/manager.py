...
class MCPClientManager:
        ...
        async with self._lock:
            ...
            if old_client is not None:
                ...
                try:
                    ...
                    self._force_close_client(old_client)
                except Exception as e:
                    error_msg = str(e)
                    if "different task" in error_msg or "cancel scope" in error_msg:
                        logger.warning(
                            f"Old MCP client '{key}' cleanup in different task context — force-closing"
                        )
                        self._force_close_client(old_client)
                    else:
    ...
    async def remove_client(self, key: str) -> None:
        ...
        if old_client is not None:
            ...
            try:
                ...
                if "different task" in error_msg or "cancel scope" in error_msg:
                    logger.warning(
                        f"MCP client '{key}' cleanup in different task context — force-closing resources"
                    )
                    self._force_close_client(old_client)
                else:
    ...
    async def close_all(self) -> None:
        ...
        Handles task-context mismatch: if clients were connected in a different
        async task (e.g., during lifespan startup), the AsyncExitStack's cancel
        scopes may fail to exit. We force-close underlying resources in that case.
        """
        async with self._lock:
        ...
        class CancelScopeFilter(logging.Filter):
            def filter(self, record):
                msg = record.getMessage()
                return "cancel scope" not in msg and "different task" not in msg
        
        agentscope_logger = logging.getLogger("agentscope.mcp._stateful_client_base")
        filter_obj = CancelScopeFilter()
        agentscope_logger.addFilter(filter_obj)
            ...
            for key, client in clients_snapshot:
                if client is not None:
                    try:
                        ...
                        if "different task" in error_msg or "cancel scope" in error_msg:
                            logger.info(
                                f"MCP client '{key}' cleaned up (task-context mismatch handled)"
                            )
                            self._force_close_client(client)
                        else:
                            logger.warning(f"Error closing MCP client '{key}': {e}")
        finally:
            agentscope_logger.removeFilter(filter_obj)

    @staticmethod
    def _force_close_client(client: Any) -> None:
        """Force-close MCP client resources when AsyncExitStack cleanup fails.
        
        Bypasses the cancel-scope-based cleanup and directly terminates
        subprocess handles and marks the client as disconnected.
        """
        try:
