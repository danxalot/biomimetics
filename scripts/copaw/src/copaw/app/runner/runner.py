...
# ... lines 215-219 ...
            ...
            # --- AUTHORITATIVE MEMORY HOOK (After-Turn) ---
            # Dedup: Shifting all persistence to this single hook
            if agent is not None:
                try:
                    # Retrieve the most recent turn (Prompt + Response) from agent memory
                    history = await agent.memory.get_memory()
                    if history:
                        # Extract the user prompt and assistant response
                        # Assuming the last 2 messages are the current turn
                        # (This is robust for multi-turn internal reasoning)
                        turn_content = ""
                        for m in history[-2:]:
                    ...
                    logger.warning(f"After-turn memory hook failed: {e}")
            # -----------------------------------------------
...
# ... lines 440-453 ...
        if os.environ.get("ENABLE_MEMORY_MANAGER", "true").lower() == "false":
            logger.info("MemoryManager disabled. Using GCP orchestrator.")
        else:
            logger.info("Local MemoryManager is deprecated in favor of stateless GCP Memory Orchestrator.")
...
# ... lines 459-463 ...
    ...
    async def _memorize_turn(
        ...
        """Forward a conversation turn to the GCP Memory Orchestrator."""
        if not content:
            return

        try:
            # 1. Fetch Auth Token (Inherit CoPaw's JIT secret fetcher)
            from ...app.routers.memory import GCP_GATEWAY_URL
            from ...app.routers.memory import get_secret_async
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if resp.status_code == 200:
                    logger.info(f"✅ Turn memorized for session {session_id[:8]}")
                else:
                    logger.warning(f"⚠️ Memory orchestrator rejection: {resp.status_code}")

        except Exception as e:
            logger.error(f"Failed to push turn to memory orchestrator: {e}")
