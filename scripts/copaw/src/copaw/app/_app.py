...
async def lifespan(
    ...
    if hasattr(config, "mcp"):
        try:
            # Degraded Mode: Parallel init with 100s global safety timeout
            # (manager.py already enforces 90s per client)
            await asyncio.wait_for(mcp_manager.init_from_config(config.mcp), timeout=100.0)
            logger.info("MCP client manager initialized (Degraded Mode check passed)")
        except asyncio.TimeoutError:
            logger.error("TIMEOUT: MCP manager initialization exceeded 100s. Proceeding in Degraded Mode.")
        except Exception as e:
            logger.error(f"Failed to initialize MCP manager: {e}. Proceeding in Degraded Mode.")
    ...
    # --- Phantom Sandbox Controller (Background Dev) ---
    phantom = get_phantom_controller()
    await phantom.start()
    ...
    async def _do_restart_services(
        ...
        if hasattr(config, "mcp"):
            try:
                # Degraded Mode: Ensure restart continues even if external MCPs are flaky
                await asyncio.wait_for(new_mcp_manager.init_from_config(config.mcp), timeout=100.0)
            except asyncio.TimeoutError:
                logger.error("restart_services: MCP init TIMEOUT (100s). Proceeding in Degraded Mode.")
            except Exception as e:
                logger.error(f"restart_services: MCP init failed: {e}. Proceeding in Degraded Mode.")
...
# Ingestion: Email, Obsidian, Generic Webhooks
