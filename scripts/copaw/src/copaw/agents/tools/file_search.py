...
def _load_ignore_patterns() -> list[str]:
    """Load ignore patterns from .serena_ignore in WORKING_DIR."""
    ignore_file = WORKING_DIR / ".serena_ignore"
    patterns = [
        ".git",
        "__pycache__",
        "node_modules",
...
def _is_ignored(path: Path, patterns: list[str]) -> bool:
    ...
    # Use absolute() instead of resolve() to avoid following symlink loops
    try:
        path_str = str(path.absolute())
    except OSError:
        # If we can't get absolute path, it might be a broken or looping link
        return True

    for pattern in patterns:
        if pattern.startswith("/") or "/" in pattern:
            # Handle absolute or relative paths with slashes
        ...
        try:
            for f in search_root.rglob("*"):
                try:
                    ...
                    # Skip files that trigger OS errors (e.g. symlink loops)
                    continue
        except OSError:
            # Handle errors at the rglob level
            pass
        files.sort()
