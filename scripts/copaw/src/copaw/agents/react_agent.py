...
# Configuration for GCP Memory Orchestrator
...
class CoPawAgent(ToolGuardMixin, ReActAgent):
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def __init__(
        ...
        # Setup memory tools (GCP-proxied)
        self._setup_memory_tools(namesake_strategy)
    ...
    def _setup_memory_tools(
        self,
        namesake_strategy: NamesakeStrategy,
    ) -> None:
        """Register memory tools (strictly GCP-proxied to enforce statelessness).

        Args:
            namesake_strategy: Strategy to handle namesake tool functions
        """
        # Register memory tools (GCP-proxied)
        self.toolkit.register_tool_function(
            create_memory_search_tool(None),
            namesake_strategy=namesake_strategy,
        )
        self.toolkit.register_tool_function(
            create_memory_write_tool(None),
            namesake_strategy=namesake_strategy,
        )
        logger.info("Registered memory tools (GCP Memory Orchestrator proxy)")
    ...
    async def _query_memory_orchestrator(
        self,
        query: str,
        user_id: str = "copaw",
        max_results: int = 3,
    ) -> str:
        """Query the GCP Memory Orchestrator for proactive context injection.

        Ensures the agent has access to relevant historical context before
        every reasoning turn by querying the GCP gateway.
        """
        try:
            logger.info("📡 [GCP_GATEWAY] Starting proactive context retrieval...")

            # --- AUTH STEP ---
            logger.info("📡 [GCP_GATEWAY] Fetching secrets from localhost:8089...")
            token = os.environ.get("SERVICE_ACCOUNT_TOKEN", "")

            # If token not in env, try local credentials server (Async)
            if not token:
                try:
                    async with httpx.AsyncClient(timeout=2.0) as client:
                        # Fixed API key and endpoint from copaw_secret_fetcher.py
                        vault_name = "service-account-token-txt"
                        CREDENTIALS_API_KEY = (
                            "4Kib2uPPgTmBmlXoYi9tDmgJH-t-1nZBIXgBMPf7ljQ"
                        )
                        ...
                        if auth_resp.status_code == 200:
                            token = auth_resp.json().get("value", "").strip()
                            logger.info(
    ...
    async def reply(
        ...
        # Inject historical context before reasoning for non-command queries
        if query:
            context = await self._query_memory_orchestrator(query)
            if context:
                if isinstance(msg, list):
                    msg[-1].content = f"{context}\n\n{msg[-1].content}"
                else:
